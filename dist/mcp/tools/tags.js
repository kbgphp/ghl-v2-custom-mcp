...
export const readTagsTool = {
    name: "read_tags",
    description: "Retrieve all available tags configured across the GoHighLevel sub-account, or inspect the specific tags assigned to a contact.",
    parameters: z.object({
...
export const applyTagsTool = {
    name: "apply_tags",
    description: "Apply one or more tags to a GoHighLevel contact (e.g. 'VIP Client', 'Follow Up Required'), with optional ability to remove existing tags.",
    parameters: z.object({
            ...
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        summary: `Updated tags for contact ${args.contactId}. Added: [${args.tags.join(", ")}]. Active tags: [${result.activeTags.join(", ")}].`,
                        source: result.source,
