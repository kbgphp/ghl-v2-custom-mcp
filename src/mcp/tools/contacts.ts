...
export const searchContactsTool = {
  ...
  description:
    "Search contacts in the GoHighLevel sub-account by text query (matches name, email, phone, company), email, phone, or tags.",
  parameters: z.object({
    query: z
      .string()
      .optional()
...
export const readContactWithCustomFieldsTool = {
  ...
  description:
    "Read comprehensive contact details including all basic fields, tags, and decoded custom fields (e.g. Lead Score, Budget, Services, Notes).",
  parameters: z.object({
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              summary: `Retrieved contact '${result.contact.name || result.contact.email}' with ${
                Object.keys(result.contact.customFieldMap || {}).length
              } custom fields and ${result.contact.tags.length} tags.`,
              source: result.source,
...
export const updateContactFieldsTool = {
  ...
  description:
    "Update standard contact fields (firstName, lastName, email, phone, companyName, address) and/or custom fields on a GoHighLevel contact.",
  parameters: z.object({
    ...
    customFieldMap: z
      ...
      .optional()
      .describe(
        "Key-value dictionary of custom fields to update (e.g. {'Lead Score': 95, 'Budget Range': '$50,000+', 'Target Start Date': '2026-10-01'})"
      ),
    locationId: z.string().optional().describe("Sub-account location ID (optional)")
  }),
  execute: async (args: {
...
export const createContactTool = {
  name: "create_contact",
  description:
    "Create a new contact in the GoHighLevel sub-account with initial standard fields, custom fields, and tags.",
  parameters: z.object({
