...
export const readCustomFieldDefinitionsTool = {
  ...
  description:
    "Retrieve all custom field definitions and metadata (field names, IDs, data types, dropdown options) configured for contacts in the sub-account.",
  parameters: z.object({
    locationId: z.string().optional().describe("Sub-account location ID (optional)")
  }),
  execute: async (args: { locationId?: string }) => {
