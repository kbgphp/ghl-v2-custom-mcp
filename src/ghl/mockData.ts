...
export class MockDataStore {
  ...
  public searchContacts(params: {
    ...
    if (params.tags && params.tags.length > 0) {
      result = result.filter((c) =>
        params.tags!.some((t) => c.tags.map((x) => x.toLowerCase()).includes(t.toLowerCase()))
      );
    }
  ...
  public getContactByEmailOrPhone(email?: string, phone?: string): Contact | undefined {
    if (email) {
      const found = this.contacts.find((c) => c.email?.toLowerCase() === email.toLowerCase());
      if (found) return this.enrichContact(found);
    }
    if (phone) {
  ...
  public createContact(data: Partial<Contact> & { email: string }): Contact {
  ...
  public updateContact(id: string, updates: Partial<Contact>): Contact | undefined {
    ...
    if (updates.customFields && updates.customFields.length > 0) {
      for (const newCf of updates.customFields) {
        const cfIndex = updatedCustomFields.findIndex(
          (cf) => (newCf.id && cf.id === newCf.id) || (newCf.key && cf.key === newCf.key)
        );
        if (cfIndex >= 0) {
  ...
  public applyTagsToContact(id: string, tagsToAdd: string[], tagsToRemove?: string[]): Contact | undefined {
    const contact = this.contacts.find((c) => c.id === id);
  ...
  public getCalendarById(id: string): Calendar | undefined {
